...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash(_('Your thoughts are shared !'))
        return redirect(url_for('main.index'))
    return render_template('index.html', title='', posts=posts.items, 
...
def user(username):
    ...
    return render_template('user.html', user=user, posts=posts.items, next_url=next_url, prev_url=prev_url, form=form, recipient=username)
...
def send_message(recipient):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(msg)
        db.session.commit()
        flash(_('Your thoughts has been shared to {}.'.format(user.username)))
